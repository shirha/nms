...
if ilog == '0' or ilog == 't':
  ...
  with open(f'pip{int(ilog) - 1}.json', "r") as infile: 
    db = json.load(infile)
if argv:
  assert '-' in argv[0], "SYNTAX: python pipj.py -f 0|1|2|t -t title -d aigtrv"
...
for dir in ["normal\\frontiers1"]: #dirs: #filter(lambda x: title.lower() in x, dirs): # ["normal\\playground"]: #["derelict-restapi"]: #["normal\\normal9"]: #["normal\\playground"]: #["expeditions\\expeditions12 omega"]: #["normal\\normal8 omega"]: #['derelict-restapi']: #["normal\\normal9"]: #["expeditions\\expeditions9 utopia"]:  #dirs: #['orbital']: #["expeditions\\expeditions10 sigularity"]: #['normal\\playground']: #["normal\\normal8 orbital"]: 
...
#         large_image = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        ...
        if re.search(r'20210910062400',path): dbug = 'igtrv'
        ...
        if not sysflag:
          ret,res,sys = m.isVisited(path,dbug,ilog,db,large_image,station)
          if ret:
            if res is not None:
              station = res
        ...
        ret = m.isTechno(path,dbug,ilog,db,large_image,station)
        if ret: 
          continue
